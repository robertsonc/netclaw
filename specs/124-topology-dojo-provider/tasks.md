...
- [ ] T006 Record the `element.upsert` operation shape from the target deployment's
...
- [ ] T014 [US1] Write the SKILL.md "Connection modes" section — hosted with a Topology Dojo API
...
- [ ] T017 [P] [US2] `test_dojo_document.py::TestDiffSourced` — fed a fixture **sourced-element
...
- [ ] T018 [US2] Implement `build_upsert_batches()` and `diff_sourced()` in `dojo_document.py`;
...
- [ ] T027 [US4] Add `build_workspace_batches()` to `dojo_document.py` emitting
  `element.upsert` operations (research R7)
- [ ] T028 [US4] SKILL.md "Propose changes to a workspace" section: manifest (stop if
...
- [ ] T041 [P] `scripts/in2n-profiles.py`: add `"topology-dojo-diagram"` to the `viz` profile's
  `exact` list
- [ ] T042 `python3 scripts/check-package-references.py --refresh` — expected to find no new
